...
export default function RolesPage() {
  ...
  const setPermsMutation = useMutation({
    mutationFn: (vars: { roleId: string; permissionIds: string[] }) =>
      fetch(`/api/v1/roles/${vars.roleId}/permissions`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${accessToken}` }, body: JSON.stringify({ permissionIds: vars.permissionIds }) }).then(ensureOk),
